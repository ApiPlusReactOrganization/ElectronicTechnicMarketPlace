...
export const manufacturerSlice = createSlice({
  ...
  reducers: {
    ...
    deleteManufacturerReduser: (state, action) => {
      state.manufacturerList = state.manufacturerList.filter(m => m.id !== action.payload.id);
    },
    updateManufacturerReducer: (state, action) => {
      const index = state.manufacturerList.findIndex(m => m.id === action.payload.id);
      if (index !== -1) {
...
export const { getAll, addManufacturer, deleteManufacturerReduser, updateManufacturerReducer } = manufacturerSlice.actions;
